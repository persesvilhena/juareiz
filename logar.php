...
if(isset($_POST["logar"])) { // Verifico se o botão de login foi acionado
	...
	if(!empty($_POST["login"]) && !empty($_POST["senha"])) { // Verifico se os campos foram preenchidos
		...
		$valida_user = mysqli_query($conecta, "SELECT * FROM $tabela WHERE nome='$login' AND senha='$senha'"); // Faço a consulta ao SQL para buscar o usuário com os dados informados pelo form
...
if(isset($_POST["cadastrar"])) { // Verifico se o botão cadastrar foi acionado
	...
	if(!empty($_POST["login"]) && !empty($_POST["senha"]) && !empty($_POST["email"])) { // Verifico se os campos foram preenchidos
		...
		$repeat_user = mysqli_query($conecta, "SELECT * FROM $tabela WHERE nome='$login'") or die($mensagem_erro = "<b>Houve um erro:</b><br />".mysqli_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
		...
		if(mysqli_num_rows($repeat_user) > 0) { // Verifico se a consulta retorna algum resultado. Nesse caso, se retornar, define uma mensagem de erro.
			...
			$insert = mysqli_query($conecta, "INSERT INTO $tabela(nome, senha, email, nome1, descricao1, nome2, foto) VALUES('$login', '$senha', '$email', '$nome1', '$descricao1', '$nome2', 'anon/anon.jpg')") or die(mysqli_error()); // Insiro os dados no BD
