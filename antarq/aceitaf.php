...
<title>Aceitar - <?php echo "$namesite"; ?></title>
...
if(isset($_POST["cadastrar"])) { // Verifico se o botão cadastrar foi acionado
	...
	if(!empty($_POST["us1"]) && !empty($_POST["us2"]) && !empty($_POST["us1name"])) { // Verifico se os campos foram preenchidos
		...
		$repeat_user = mysqli_query($conecta, "SELECT * FROM amigos WHERE us1='$us1' AND us2='$us2';") or die($mensagem_erro = "<b>Houve um erro:</b><br />".mysqli_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
		...
		if(mysqli_num_rows($repeat_user) > 0) { // Verifico se a consulta retorna algum resultado. Nesse caso, se retornar, define uma mensagem de erro.
			...
			$insert = mysqli_query($conecta, "insert into amigos values (null,'$us2','$us1','$us2name','$us1name');") or die(mysqli_error()); // Insiro os dados no BD
			$insert = mysqli_query($conecta, "insert into amigos values (null,'$us1','$us2','$us1name','$us2name');") or die(mysqli_error()); // Insiro os dados no BD
			$insert = mysqli_query($conecta, "delete from convite2 where deid='$us2' and paraid='$us1';") or die(mysqli_error()); // Insiro os dados no BD
