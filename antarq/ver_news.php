...
if(isset($_POST["cadastrar"])) { // Verifico se o botão cadastrar foi acionado
	...
	if(!empty($_POST["msg"]) && !empty($_POST["uid"])) { // Verifico se os campos foram preenchidos
		...
		$repeat_user = mysqli_query($conecta, "SELECT * FROM del WHERE us1='$msg'") or die($mensagem_erro = "<b>Houve um erro:</b><br />".mysqli_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
		...
		if(mysqli_num_rows($repeat_user) > 0) { // Verifico se a consulta retorna algum resultado. Nesse caso, se retornar, define uma mensagem de erro.
			...
			$insert = mysqli_query($conecta, "INSERT INTO com_news VALUES(null, '$id_gen', '$login', '$msg', '$uid')") or die(mysqli_error()); // Insiro os dados no BD
...
$querysql = mysqli_query($conecta, "SELECT * FROM news WHERE id='$uid'");
$data = mysqli_fetch_array($querysql);
$querysql1 = mysqli_query($conecta, "SELECT * FROM com_news WHERE uid='$uid'");
$data1 = mysqli_fetch_array($querysql1);
?>
...
$res = mysqli_query($conecta, "SELECT * FROM `com_news` where newsid=$uid ORDER BY id DESC LIMIT 0,100;"); /*Executa o comando SQL, no caso para pegar todos os usuarios do sistema e retorna o valor da consulta em uma variavel ($res)  */
