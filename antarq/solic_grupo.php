...
if(isset($_POST["cadastrar"])) {
	...
	if(!empty($_POST["paraid"]) && !empty($_POST["nome"])) {
		...
		$repeat_user = mysqli_query($conecta, "SELECT * FROM us_grupo WHERE us='$id_gen'") or die($mensagem_erro = "<b>Houve um erro:</b><br />".mysqli_error());
		...
		if(mysqli_num_rows($repeat_user) > 0) {
			...
			$insert = mysqli_query($conecta, "INSERT INTO solic_grupo values(null, '$id_gen', '$paraid', '$login')") or die(mysqli_error());
