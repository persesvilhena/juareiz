...
if(isset($_POST["cadastrar"])) { // Verifico se o botão cadastrar foi acionado
	...
	if(!empty($_POST["us1"]) && !empty($_POST["us1name"])) { // Verifico se os campos foram preenchidos
		...
		$repeat_user = mysqli_query($conecta, "SELECT * FROM us_grupo WHERE us='$id_gen';") or die($mensagem_erro = "<b>Houve um erro:</b><br />".mysqli_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
		$repeat_user1 = mysqli_query($conecta, "SELECT * FROM users1 WHERE id='$id_gen' AND senha='$senha';") or die($mensagem_erro = "<b>Houve um erro:</b><br />".mysqli_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
		
		if(mysqli_num_rows($repeat_user1) > 0) { // Verifico se a consulta retorna algum resultado. Nesse caso, se retornar, define uma mensagem de erro.
		...
		if(mysqli_num_rows($repeat_user) < 0) { // Verifico se a consulta retorna algum resultado. Nesse caso, se retornar, define uma mensagem de erro.
			...
			$insert = mysqli_query($conecta, "delete from us_grupo where us='$id_gen';") or die(mysqli_error()); // Insiro os dados no BD
