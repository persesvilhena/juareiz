...
$insert = mysqli_query($conecta, "INSERT INTO com_post VALUES(null, '$uid1', '$id_gen', '$row[nome]', '$msg')") or die(mysqli_error());
if($insert) 
...
$querysql = mysqli_query($conecta, "SELECT * FROM post WHERE id='$uid'");
...
<?php include "includes/cb.dll"; ?>
<?php
$res = mysqli_query($conecta, "SELECT * FROM `com_post` where id=$uid ORDER BY id DESC LIMIT 0,100;"); /*Executa o comando SQL, no caso para pegar todos os usuarios do sistema e retorna o valor da consulta em uma variavel ($res)  */
