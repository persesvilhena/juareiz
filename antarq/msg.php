...
if(isset($_POST["cadastrar"])) { // Verifico se o botão cadastrar foi acionado
	...
	if(!empty($_POST["assunto"]) && !empty($_POST["msg"]) && !empty($_POST["nome"])) { // Verifico se os campos foram preenchidos
		...
		$repeat_user = mysqli_query($conecta, "SELECT * FROM msg WHERE nome='$msg'") or die($mensagem_erro = "<b>Houve um erro:</b><br />".mysqli_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
		...
		if(mysqli_num_rows($repeat_user) > 0) { // Verifico se a consulta retorna algum resultado. Nesse caso, se retornar, define uma mensagem de erro.
			...
			$insert = mysqli_query($conecta, "INSERT INTO msg(deid, paraid, assunto, msg, nome) VALUES('$de', '$para', '$assunto', '$msg', '$nome')") or die(mysqli_error()); // Insiro os dados no BD
